...
<!--                            </template>-->
                            ...
                            <template #footer>
                                <div style="display:flex;flex-direction: column;text-align: left;">
                                    ...
                                    <div>
                                        送货地址:{{item.province}}{{item.city}}{{item.county}}{{item.deliveryAddress}}
                                    </div>
                                </div>
                            </template>
                        </van-card>
            ...
            <van-popup v-model="boxShow" position="bottom" closeable :style="{ height: '90%' }" @closed="chanel">
                <van-cell-group>
                    <van-form validate-first>
                        ...
                        <van-field label="客户编号" placeholder="请填写客户编号" v-model="submitForm.number"  required :rules="[{ required: true, message: '请填写客户编号' }]" />
                        <van-field label="客户名称" placeholder="请填写客户名称" v-model="submitForm.name" required :rules="[{ required: true, message: '请填写客户名称' }]" />
                        <van-field label="客户类别" placeholder="请点击选择分类" v-model="submitForm.cCategoryName" clickable readonly @click="typeBoxShow = true;" />
                        ...
                        <van-field label="客户等级" placeholder="请点击选择等级" v-model="submitForm.cLevelName" clickable readonly @click="levelShow = true;" />
                        <van-popup v-model="levelShow" position="bottom">
                            <van-picker
                                show-toolbar
                                :columns="levelList"
                                value-key="name"
                                @confirm="(v)=>onConfirm(v,2)"
...
export default {
    ...
    methods:{
        ...
        onSubmit(){
            ...
            if(!me.submitForm.id){
                api = me.addApi;
                me.submitForm.amount = me.submitForm.amount > 0 ? me.submitForm.amount : 0;
                me.submitForm.periodMoney = me.submitForm.periodMoney > 0 ? me.submitForm.periodMoney : 0;
            }
            let para = JSON.parse(JSON.stringify(me.submitForm));
            if(para.linkMans){
                para.linkMans = JSON.stringify(para.linkMans);
            }
            axios.post(api, para).then(res => {
        ...
        getTypeList(){//获取客户类别列表
            const me = this;
            axios.get(me.typeApi, {params:{typeNumber:'customertype',name:'客户',isDelete:2,pageSize:5000}}).then(res => {
