                ...
                >
                    <van-swipe-cell v-for="(item,index) in list" :key="index" style="margin-bottom: 5%;">
                        <van-card
                            ...
                            :desc="'供应商：'+item.contactName"
                            :title="'其他入库单编号：'+item.billNo"
                            class="goods-card"
                            @click="edit(item)"
                        >
                            <template #tags>
            ...
            <div style="margin-top:15%;">
                ...
                <van-form validate-first>
                    ...
                    <van-field
                        ...
                        @click="$refs.contactSelect.open()"
                    />
                    <contact-select ref="contactSelect" @select="contactSelect"></contact-select>
                    <van-field
                        readonly
                        clickable
                    ...
                    />
                    <goods-select ref="goodsSelect" @query="queryGoods"></goods-select>
...
import ContactSelect from "../components/common/contact-select";
export default {
    name: "inbound",
    components:{
        GoodsSelect,
    ...
    data(){
        return {
            ...
            addApi:'/api/invOi/addNew',
            editApi:'/api/invOi/edit',
            listApi:'/api/invOi/list',
            infoApi:'/api/invOi/info',
        }
    },
    methods: {
        contactSelect(select){//供应商选中
        ...
        async getBillNo(){
            const me = this;
            let rep = await axios.get('/api/getListCode/QTRK', {});
        ...
        async onSubmit() {
            ...
            para.transTypeName = '其他入库';
            para.transTypeId = 150706;
